...
docs = []
metadata = []   # <--- neu: Buch- und Kapitelinformationen
...
for i in range(1, 6):
    ...
    for chapter in chapter_files:
        ...
        with open(path, "r", encoding="utf-8") as f:
            ...
            # === Manuelle Stopwords ===
            custom_stopwords = {
                "in",  "ein", "wie", "10", "20", "21", "nach", "im", "für", "gewesen", "nicht", "nachher",
                "mit", "oder", "allein", "alles", "alle", "allerdings", "auf", "auch", "aber", "aus", "an", "als",
                "bald", "diese", "dies", "die", "dieser", "der", "das", "daß", "dem", "den", "des", "durch", "darauf",
                "er", "es", "hatte", "sich", "sein", "ist", "um", "und", "von", "wieder", "war", "ward", "wäre",
                "wären", "zu"
            }
            ...
            metadata.extend([(f"Buch{i}", chapter)] * len(segments))  # <--- neu
...
# === Statistische Verteilung (gesamt) ===
try:
...
# === Variante B: Themenverteilung nach Buch (neu) ===
try:
    print("📊 Erstelle Balkendiagramm der Themenverteilung nach Buch ...")
    df = pd.DataFrame({
        "Topic": topics,
        "Buch": [b for (b, k) in metadata],
        "Kapitel": [k for (b, k) in metadata]
    })

    topic_counts = df.groupby(["Buch", "Topic"]).size().unstack(fill_value=0)

    topic_counts.plot(kind="bar", stacked=True, figsize=(12, 6))
    plt.title("Themenverteilung nach Buch (Mommsen, Römische Geschichte)")
    plt.xlabel("Buch")
    plt.ylabel("Anzahl Segmente")
    plt.tight_layout()
    plt_path_buch = os.path.join(output_folder, "Topic_Distribution_per_Book.png")
    plt.savefig(plt_path_buch)
    print(f"✅ Balkendiagramm gespeichert unter: {plt_path_buch}")
except Exception as e:
    print(f"⚠️ Keine Themenverteilung nach Buch erstellt: {e}")

print("\n✅ Fertig! Topic-Modelling und Themenverteilungen abgeschlossen.")
